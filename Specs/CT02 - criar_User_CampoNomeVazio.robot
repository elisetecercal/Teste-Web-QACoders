...
Realizar Login e Cadastrar novo Usuario Com Nome Vazio
    ...
    Input Text     ${CadastrosUsuario.NomeCompletoUsuario}  ${ValorVazio}
    ...
    # Passo 5: Verifica se a mensagem de erro é exibida
    Element Should Be Visible    xpath=//p[contains(@class, 'css-bbipig') and contains(text(), 'O campo nome completo é obrigatório')]

    # Verifica se o texto da mensagem de erro é o esperado
    Element Text Should Be    xpath=//p[contains(@class, 'css-bbipig') and contains(text(), 'O campo nome completo é obrigatório')]    O campo nome completo é obrigatório
